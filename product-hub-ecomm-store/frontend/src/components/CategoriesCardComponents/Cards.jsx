    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
      {categories.map((slug) => (
        <Link key={slug} to={`/category/${slug}`}>
          <div className="bg-white border border-gray-100 shadow-md rounded-2xl p-5 hover:shadow-xl hover:-translate-y-2 transition-all duration-200 text-center group cursor-pointer flex flex-col items-center justify-center min-h-30">
            {/* Emoji icon */}
